...
<script>
import { getSysMessageList, delSysMessageList } from '@/api/table'
export default {
  data() {
    return {
      total: 12,
  ...
  methods: {
    ...
    handleEdit(index, row) { // 编辑按钮
      this.$router.push({ name: 'sysMessageDetail', params: { id: row.id }}) // 跳转消息详情页
    },
    handleDelete(index, row) { // 删除按钮
      // 删除成功以后，重新加载列表
      delSysMessageList(row.id).then(resopnse => {
        this.getlist()
      })
